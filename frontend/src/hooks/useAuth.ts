...
export function useAuth() {
  const { user, isAuthenticated, setUser, setLoading, setError, reset } =
    useAppStore();

  useEffect(() => {
    const checkAuth = async () => {
      // Check for vibe_token in URL (passed from preview iframe)
      const urlParams = new URLSearchParams(window.location.search);
      const vibeToken = urlParams.get("vibe_token");
      if (vibeToken) {
        localStorage.setItem("auth_token", vibeToken);
        // Clean up URL
        urlParams.delete("vibe_token");
  ...
  const signIn = useCallback(async () => {
    ...
    try {
      ...
      const options: SignInWithAppleOptions = isNative
        ...
        : {
            clientId:
              import.meta.env.VITE_APPLE_CLIENT_ID || "com.crosswalk.app.web",
            redirectURI:
